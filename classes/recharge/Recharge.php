...
class Recharge extends FirstClass
{
    ...
    {
        $this->model = new \app\recharge\model\Recharge();
    }
    ...
    public function edit($id)
    {
        ...
        if (is_null($model)) parent::redirect_exception('/recharge', ['订单不存在']);
    ...
    private function integralAdd($order)
    {
        ...
        $member->jpj += $order['jpj'];
        $member->jpj_all += $order['jpj'];
        $member->save();
        ...
        $record->jpj = $order['jpj'];
        $record->jpj_now = $member->jpj;
        $record->jpj_all = $member->jpj_all;
        $record->type = '20';
        $record->content = '管理员处理了您的直充订单(订单号：' . $order['order_number'] . ')，兑换『' . $set['webAliasJpj'] . '』：' . $order['jpj'];
